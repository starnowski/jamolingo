package com.github.starnowski.jamolingo.compat.driver.operators.skip;
...
public class SkipOperatorTest extends AbstractItTest {
  ...
  public void shouldReturnExpectedNumberOfDocuments(int skipValue, int expectedCount)
      throws Exception {
    // GIVEN
    ...
    UriInfo uriInfo =
        new Parser(edm, OData.newInstance()).parseUri("Items", "$skip=" + skipValue, null, null);
    OdataSkipToMongoSkipParser parser = new OdataSkipToMongoSkipParser();

    // WHEN
    SkipOperatorResult result = parser.parse(uriInfo.getSkipOption());
  ...
  private static Stream<Arguments> provideShouldReturnExpectedNumberOfDocuments() {
    return Stream.of(Arguments.of(1, 2), Arguments.of(2, 1), Arguments.of(3, 0));
  }
}
