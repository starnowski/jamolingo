...
class ODataFilterToMongoMatchParserWithOverrideConfigurationTest extends AbstractSpecification {
    ...
    static oneToOneEdmPathsMappings() {
        [
                ...
                [ "numericArray/all(n:n eq 10 or n eq 20 or n eq 30)", """{"\$match": {"\$and": [{"\$expr": {"\$eq": [{"\$size": {"\$filter": {"input": {"\$ifNull": ["\$renamed_numericArray", []]}, "as": "n", "cond": {"\$or": [{"\$or": [{"\$eq": ["\$\$n", 10]}, {"\$eq": ["\$\$n", 20]}]}, {"\$eq", 30]}]}}}}, {"\$size": {"\$ifNull": ["\$renamed_numericArray", []]}}]}}]}}""", ["renamed_numericArray"]],
